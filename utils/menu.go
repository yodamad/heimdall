// Package utils : Inspired from https://github.com/Nexidian/gocliselect
package utils
...
func (m *Menu) renderMenuItems(redraw bool) {
	...
	for index, menuItem := range m.MenuItems {
		...
		menuItemText := colorstring.Color(menuItem.Text)
		cursor := "[ ]"
		if index == m.CursorPos {
			cursor = colorstring.Color("[yellow][X]")
			menuItemText = colorstring.Color("[yellow]" + menuItemText)
		}
...
func (m *Menu) Display() string {
	...
	fmt.Println(colorstring.Color("[cyan][bold]" + m.Prompt + "[reset]:"))
